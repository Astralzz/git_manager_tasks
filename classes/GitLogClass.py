...
class GitLogClass:
    ...
    # Funcion para asegurar que exista el directorio de logs
    ...
    # Funcion para obtener el nombre del archivo de log para hoy
    ...
    # Funcion para obtener la ruta completa del archivo de log de hoy
    ...
    # Funcion para registrar una operacion en el log diario
    def log_operation(self, operation: str, details: str = "", status: str = "INFO") -> None:
        """
        Registra una operación en el log diario
        @param {str} operation: Nombre de la operación
        @param {str} details: Detalles adicionales
        @param {str} status: Estado de la operación (INFO, SUCCESS, WARNING, ERROR)
        """
    ...
    # Funcion para registrar un comando git ejecutado
    def log_git_command(self, command: str, result: dict) -> None:
        """
        Registra un comando Git ejecutado
        @param {str} command: Comando ejecutado
        @param {dict} result: Resultado del comando
        """
    ...
    # Funcion para registrar la seleccion de una opcion del menu
    ...
    # Funcion para registrar una entrada del usuario
    ...
    # Funcion para registrar operaciones relacionadas con ramas
    ...
    # Funcion para registrar operaciones de rebase
    def log_rebase_operation(self, base_branch: str, feature_branch: str, status: str = "INFO") -> None:
        """
        Registra operaciones de rebase
        @param {str} base_branch: Rama base
        @param {str} feature_branch: Rama feature
        @param {str} status: Estado de la operación
        """
        details = f"From: {base_branch} → To: {feature_branch}"
        self.log_operation("REBASE", details, status)
    
    # Funcion para registrar operaciones de pull
    def log_pull_operation(self, branch_name: str, status: str = "INFO") -> None:
        """
    ...
    # Funcion para registrar operaciones de push
    def log_push_operation(self, branch_name: str, commit_message: str, status: str = "INFO") -> None:
        """
    ...
    # Funcion para registrar operaciones de stash
    def log_stash_operation(self, operation: str, stash_message: str = "", status: str = "INFO") -> None:
        """
    ...
    # Funcion para registrar advertencias
    ...
    # Funcion para registrar operaciones exitosas
    ...
    # Funcion para registrar el inicio del programa con la configuracion seleccionada
    def log_program_start(self, config: dict) -> None:
        """
        Registra el inicio del programa con la configuración seleccionada
        @param {dict} config: Configuración seleccionada
        """
        ...
        try:
            with open(log_file_path, "a", encoding="utf-8") as log_file:
                log_file.write(f"\n{separator}\n")
                log_file.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] {start_message}\n")
                log_file.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] CONFIG_SELECTED - {config_info}\n")
                log_file.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] PROJECT_INFO - {project_info}\n")
                log_file.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] SECTION_INFO - {section_info}\n")
                log_file.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] TASK_INFO - {task_info}\n")
                log_file.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] REPO_INFO - {repo_info}\n")
                log_file.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] BRANCH_INFO - {base_branch_info} | {feature_branch_info}\n")
                log_file.write(f"{separator}\n")
        except Exception as e:
            print(f"⚠️ No se pudo escribir el log de inicio: {e}")
    
    # Funcion para registrar el fin del programa
    def log_program_end(self) -> None:
        ...
        try:
            with open(log_file_path, "a", encoding="utf-8") as log_file:
                log_file.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [INFO] {end_message}\n")
                log_file.write(f"{separator}\n\n")
        except Exception as e:
            print(f"⚠️ No se pudo escribir el log de fin: {e}")
    
    # Funcion para obtener la ruta del archivo de log de hoy
    ...
    # Funcion para leer el contenido del log de hoy
    ...
    # Funcion para obtener la ruta del directorio de logs
